package com.abahstudio.startactivityforresult
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btn_launch_activity_secondary.setOnClickListener {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (resultCode == Activity.RESULT_OK){
            if (requestCode == FIRST_ACTIVITY_REQUEST_CODE){
                tv_first_acitvity_result.text="First Activity Result Success"
            }else if (requestCode == SECOND_ACTIVITY_REQUEST_CODE){
                if (data!=null){
                    val name = data.getStringExtra(NAME)
                    val email = data.getStringExtra(EMAIL)
                    tv_second_t_acitvity_result.text="$name => $email"
                }
            }
